...
export const Searchbar = () => {
  ...
  const handlechange = (e) => {
    // debounce(searchdata(e.target.value),5000)
  ...
  const searchdata = () => {
    ...
    let newarr = initialdata.productdata.filter((el) => {
     return ((el.name).toLocaleLowerCase()||(el.type).toLowerCase()).indexOf(query)!==-1?true:false
    })
    console.log(newarr)
  }
  ...
  return (
      <div className={styles.searchbar_main_container}>
          <label>
              <input placeholder='Search for brand name and another types' type="search" onChange={handlechange}></input>
              <BsSearch/>
          </label>
