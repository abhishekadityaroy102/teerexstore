...
const Singlecartbox = ({ value }) => {
  ...
  const handlechange_cart_item_qty = (e, value) => {
    if (value.quantity < e.target.value) {
      ...
      dispatchcartfunc("direct_change_cartitem", {
        num: e.target.value,
        id: value.id,
      });
    }
    ...
    <div className={styles.Singlecartbox_main_container}>
      ...
      <div className={styles.cartbox_qty_container}>
        ...
        <label>
          Qty :
          <select
            value={value.item_incart}
            onChange={(e) => handlechange_cart_item_qty(e, value)}
          >
      ...
      <div
        className={styles.delete_cart_item_button}
        onClick={() => handlecartdelete(value)}
      >
        <ImCross />
      </div>
    </div>
