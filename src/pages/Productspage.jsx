...
const Productspage = () => {
  ...
  return (
    <div className={styles.Productspage_main_container}>
      {initialdata.filterdata.length > 0 ? (
        initialdata.filterdata.length > 0 &&
        initialdata.filterdata.map((el) => (
          <Singleproductbox key={el.id} value={el} />
        ))
      ) : (
        <img className={styles.imageof_nodatafound} src={notfoundimage} />
      )}
    </div>
  );
};
